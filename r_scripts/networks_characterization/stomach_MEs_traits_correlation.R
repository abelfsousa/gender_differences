...
males_tumour_network_MEs <- read_tsv("./files/stomach_males_tumour_network_MEs_traits.txt")
females_tumour_network_MEs <- read_tsv("./files/stomach_females_tumour_network_MEs_traits.txt")
...
males_MEs_hist_type <- males_tumour_network_MEs %>%
  group_by(network, moduleL, moduleN, state) %>%
  do(broom::tidy(cor.test(.$ME, .$Lauren_Class, method = "pearson"))) %>%
  ungroup()
