...
diff_expr_normal_table <- read_tsv("./files/diff_expr_thyroid_normal_tcga_maleVSfemale_edgeR.txt") %>%
  dplyr::rename(adj.P.Val = FDR) %>%
  mutate(fdr = if_else(adj.P.Val < 0.05, "FDR < 0.05", "FDR >= 0.05"))
...
write.table(tumour_normal_signf_degs, "./files/thyroid_tumour_normal_signf_degs2.txt", sep="\t", quote=F, row.names=F)
...
males_females_signf_degs_venn <- draw.pairwise.venn(
    ...
    cex = rep(2.5, 3),
    cat.cex = rep(1.5, 2))
ggsave(filename="thyroid_tumour_normal_signf_degs_venn2.png", plot=males_females_signf_degs_venn, path = "./plots/diff_expression_maleVSfemale_tcga_normal/", width=3, height=3)
unlink("thyroid_tumour_normal_signf_degs_venn2.png")
...
ggsave(filename="diff_expr_thyroid_all_tcga_maleVSfemale.png", plot=diff_expr_vp, path = "./plots/diff_expression_maleVSfemale_tcga_normal/", width=10, height=5)
...
all_diff_genes <- tumour_normal_signf_degs %>%
  ...
  unnest()
write.table(all_diff_genes, "./files/thyroid_tumour_normal_degs_enr2.txt", sep="\t", quote=F, row.names=F)
...
tn_enr_bp <- all_diff_genes %>%
  ...
  facet_grid(Description ~ state,
    ...
    scales = "free",
    labeller=labeller(
      state = c("common" = "Common", "tumour_specific" = "Tumour-specific"),
      Description = c("GO_BP" = "GO BP", "KEGG" = "KEGG", "ONCO" = "Onco", "IMMUNO" = "Immuno", "POS" = "Pos", "CM" = "Cancer\nmodules"))) +
  theme(
...
ggsave(filename="thyroid_tumour_normal_enr_bp.png", plot=tn_enr_bp, path="./plots/diff_expression_maleVSfemale_tcga_normal/", width = 11, height = 5)
ggsave(filename="thyroid_tumour_normal_enr_bp.pdf", plot=tn_enr_bp, path="./plots/diff_expression_maleVSfemale_tcga_normal/", width = 11, height = 5)
unlink("thyroid_tumour_normal_enr_bp.png")
unlink("thyroid_tumour_normal_enr_bp.pdf")
...
degs_chr_bp <- tumour_normal_signf_degs %>%
  ...
  scale_y_continuous(name = "Number of genes")
ggsave(filename="thyroid_degs_chr_bp.png", plot=degs_chr_bp, path="./plots/diff_expression_maleVSfemale_tcga_normal/", width = 4, height = 5)
ggsave(filename="thyroid_degs_chr_bp.pdf", plot=degs_chr_bp, path="./plots/diff_expression_maleVSfemale_tcga_normal/", width = 4, height = 5)
...
save(list=ls(), file="r_workspaces/tcga_thyroid_diffExpr_malesVSfemales_characterization.RData")
