...
# PCA analysis
# batch effects regressed-out from stomach gtex data
...
stomach_gtex_fpkm_covReg <- read_tsv("./files/stomach_tcga_gtex_fpkm_GTEX_regressed_out_COVARS.txt") %>%
  column_to_rownames(var = "gene")

thyroid_gtex_fpkm_covReg <- read_tsv("./files/thyroid_tcga_gtex_fpkm_GTEX_regressed_out_COVARS.txt") %>%
  column_to_rownames(var = "gene")


stomach_tcga <- stomach_tcga_gtex_counts[, colnames(stomach_tcga_gtex_counts) %in% stomach_tcga_gtex_meta[stomach_tcga_gtex_meta$data == "TCGA", "sample"]]
thyroid_tcga <- thyroid_tcga_gtex_counts[, colnames(thyroid_tcga_gtex_counts) %in% thyroid_tcga_gtex_meta[thyroid_tcga_gtex_meta$data == "TCGA", "sample"]]

all.equal(rownames(stomach_tcga), rownames(stomach_gtex_fpkm_covReg))
all.equal(rownames(thyroid_tcga), rownames(thyroid_gtex_fpkm_covReg))


stomach_tcga_gtex_counts <- cbind(stomach_tcga, stomach_gtex_fpkm_covReg)
thyroid_tcga_gtex_counts <- cbind(thyroid_tcga, thyroid_gtex_fpkm_covReg)
...
pca_bp <- rbind(pca_thyroid, pca_stomach) %>%
  ...
  ggplot(mapping=aes(x=PC1, y=PC2, color=gender, shape=sample_type) ) +
    ...
    labs(x = "PC1 (Stomach: 76.8%, Thyroid: 85.1%)", y = "PC2 (Stomach: 5.4%, Thyroid: 2.4%)")
ggsave(filename="pca_thyroid_stomach_samples_gtex_stomachRegOut.pdf", plot=pca_bp, path = "./plots/pre_processing/", width = 8, height = 4)
unlink("pca_thyroid_stomach_samples_gtex_stomachRegOut.pdf")



# PCA stomach gtex
pca_stomach_gtex <- prcomp(t(stomach_gtex_fpkm_covReg), center = TRUE, scale. = TRUE)$x %>%
  as.data.frame()
...
pca_stomach_gtex_bp1 <- pca_stomach_gtex %>%
  ...
  #coord_fixed() +
  labs(x = "PC1", y = "PC2", title = "Stomach GTEx\nAfter regress-out the covariates")
ggsave(filename="pca_stomach_gtex_RegOut1.pdf", plot=pca_stomach_gtex_bp1, path = "./plots/pre_processing/", width = 4, height = 4)
unlink("pca_stomach_gtex_RegOut1.pdf")
...
ggsave(filename="pca_stomach_gtex_RegOut2.pdf", plot=pca_stomach_gtex_bp2, path = "./plots/pre_processing/", width = 4, height = 4)
unlink("pca_stomach_gtex_RegOut2.pdf")




# PCA thyroid gtex
pca_thyroid_gtex <- prcomp(t(thyroid_gtex_fpkm_covReg), center = TRUE, scale. = TRUE)$x %>%
  as.data.frame()
...
ggsave(filename="pca_thyroid_gtex_RegOut1.pdf", plot=pca_thyroid_gtex_bp1, path = "./plots/pre_processing/", width = 4, height = 4)
unlink("pca_thyroid_gtex_RegOut1.pdf")
...
ggsave(filename="pca_thyroid_gtex_RegOut2.pdf", plot=pca_thyroid_gtex_bp2, path = "./plots/pre_processing/", width = 4, height = 4)
unlink("pca_thyroid_gtex_RegOut2.pdf")
...
ggsave(filename="pca_thyroid_stomach_gtex_samples_RegOut1.pdf", plot=pca_thyroid_stomach_gtex, path = "./plots/pre_processing/", width = 8, height = 4)
unlink("pca_thyroid_stomach_gtex_samples_RegOut1.pdf")
...
ggsave(filename="pca_thyroid_stomach_gtex_samples_RegOut2.pdf", plot=pca_thyroid_stomach_gtex, path = "./plots/pre_processing/", width = 8, height = 4)
unlink("pca_thyroid_stomach_gtex_samples_RegOut2.pdf")
