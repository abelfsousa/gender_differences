...
network <- tibble(gene = names(males_tumour_network$colors), module = unname(males_tumour_network$colors), color = labels2colors(males_tumour_network$colors))
network <- network %>%
  filter(color != "grey") %>%
  arrange(module)

cor_matrix <- males_tumour[, network$gene]
cor_matrix <- cor(cor_matrix)
cor_matrix <- abs(cor_matrix)

pdf(file="./plots/wgcna_networks/wgcna_stomach_tumours_males_heatmap.pdf", w=6, h=5)
