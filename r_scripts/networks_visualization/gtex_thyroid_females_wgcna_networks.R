...
network <- tibble(gene = names(females_normal_network$colors), module = unname(females_normal_network$colors), color = labels2colors(females_normal_network$colors))
network <- network %>%
  filter(color != "grey") %>%
  arrange(module)

cor_matrix <- females_normal[, network$gene]
cor_matrix <- cor(cor_matrix)
cor_matrix <- abs(cor_matrix)

pdf(file="./plots/wgcna_networks/wgcna_thyroid_normal_females_heatmap.pdf", w=6, h=5)
