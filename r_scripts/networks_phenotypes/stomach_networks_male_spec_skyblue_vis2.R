...
comparing_distribution_boxpl <- ggplot(data = comparing_distribution, mapping = aes(x = weight, fill = gender, color = gender)) +
  ...
  annotate("text", x = 0.8, y = 2.1, label = "Wilcoxon,\nP-value < 2.2e-16", size = 6.5) +
  theme_classic() +
...
ggsave(filename="stomach_skyblue_module_cor_dist.png", plot = comparing_distribution_boxpl, path = "./plots/wgcna_networks_traits", width=7, height=4)
ggsave(filename="stomach_skyblue_module_cor_dist.pdf", plot = comparing_distribution_boxpl, path = "./plots/wgcna_networks_traits", width=7, height=4)
unlink("stomach_skyblue_module_cor_dist.png")
unlink("stomach_skyblue_module_cor_dist.pdf")
