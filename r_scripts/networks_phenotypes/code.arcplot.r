...
# ex:
# bash ./bash_scripts/text_to_gml.sh ./files/thyroid_greenyellow_females_mean_expression.txt ./files/thyroid_greenyellow_module_females_edges.txt ./files/thyroid_greenyellow_module_females_edges.gml
...
pdf(args[2],w=16,h=8)
par(oma=c(4,0,0,0))
arcplot(edgelist, vertices = get.vertex.attribute(mis_graph)$id, ordering=new_ord, labels=vlabels,  cex.labels=1.2,
  show.nodes=TRUE, col.nodes=vborders, bg.nodes="#de2d26", col.labels="darkblue", cex.nodes = (log(degrees)+0.5)*2,
